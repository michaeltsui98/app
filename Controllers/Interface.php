...
class Controllers_Interface extends Controllers_Base {
    ...
    public function __construct(){
     ...
     public function init(){
         ...
         if(!$app_key){
             $this->echoJson('error', 'app key 为空',array(),100);
         }
        ...
        /* if($app_key != md5('71715.cn')){
         ...
         if(!$this->c or !$this->m){
             $this->echoJson('error', '请求的接口与方法名不能为空',array(),400);
         }
      ...
      * @return boolean
      */
     public function initm(){
         $access_token = $this->getVar('access_token');
         if(!$access_token){
     ...
     public function indexAction(){
         ...
         if($this->getVar('app_key')){
             $this->init();
         }elseif($this->getVar('access_token')){
             $this->initm();
         }
