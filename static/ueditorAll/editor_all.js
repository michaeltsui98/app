(function () {
    ...
    */
    ...
    UE.plugins['video'] = function () {
         ...
         */
        function creatInsertStr(url, width, height, align, toEmbed, addParagraph, toUpload) {
            var a;
            if (toUpload == "true") {
                a = !toEmbed ?
          (addParagraph ? ('<p ' + (align && align != "none" ? (align == "center" ? ' style="text-align:center;" ' : ' style="float:"' + align) : '') + '>') : '') +
          '<img toUpload="' + toUpload + '" align="' + align + '" width="' + width + '" height="' + height + '" _url="' + url + '" class="edui-faked-video"' +
          ...
          (addParagraph ? '</p>' : '')
          :
          '<embed toupload="false" type="application/x-shockwave-flash" class="edui-faked-video" pluginspage="http://www.macromedia.com/go/getflashplayer" src="' + commonParams.dodoDevPath + '/statics/ckplayer/ckplayer.swf?f=' + url + '&i=' + commonParams.dodoDevPath + '/statics/ckplayer/loading.jpg&d=' + commonParams.dodoDevPath + '/statics/ckplayer/loading.jpg&u=&l=' + commonParams.dodoDevPath + '/statics/ckplayer/loading.jpg&r=&t=5&y=&z=&e=2&v=80&p=0&h=3&q=&m=0&o=&w=&g=&j=&k=&n=&my_url=' + commonParams.dodoDevPath + '" width="' + width + '" height="' + height + '" align="none" wmode="transparent" play="true" loop="false" menu="false" allowscriptaccess="never" allowfullscreen="true">';
            }
            else {
                a = !toEmbed ?
    (addParagraph ? ('<p ' + (align && align != "none" ? (align == "center" ? ' style="text-align:center;" ' : ' style="float:"' + align) : '') + '>') : '') +
    ...
    (addParagraph ? '</p>' : '')
    :
    '<embed toUpload="' + toUpload + '" type="application/x-shockwave-flash" class="edui-faked-video" pluginspage="http://www.macromedia.com/go/getflashplayer"' +
    ...
    ' wmode="transparent" play="true" loop="false" menu="false" allowscriptaccess="never" allowfullscreen="true" >';
        ...
        function switchImgAndEmbed(img2embed) {
            var tmpdiv,
                nodes = domUtils.getElementsByTagName(me.document, "img");
            for (var i = 0, node; node = nodes[i++];) {
                ...
                tmpdiv.innerHTML = creatInsertStr(img2embed ? node.getAttribute("_url") : node.getAttribute("src"), node.width, node.height, align, img2embed, null, node.getAttribute("toUpload"));
        ...
        me.commands["insertvideo"] = {
            execCommand: function (cmd, videoObjs, toUpload) {//toUpload判断是否是本地上传或是从网盘选取
                videoObjs = utils.isArray(videoObjs) ? videoObjs : [videoObjs];
                var html = [];
                for (var i = 0, vi, len = videoObjs.length; i < len; i++) {
                    vi = videoObjs[i];
                    html.push(creatInsertStr(vi.url, vi.width || 420, vi.height || 280, vi.align || "none", false, true, toUpload));
                }
                me.execCommand("inserthtml", html.join(""));
            },
            queryCommandState: function () {
    ...
    */
    ...
    UE.plugins['music'] = function () {
        ...
        function creatInsertStr(url, width, height, align, toEmbed, addParagraph, toUpload) {
            ...
            if (toUpload == "true") {
                ...
                (addParagraph ? ('<p ' + (align != "none" ? (align == "center" ? ' style="text-align:center;" ' : ' style="float:"' + align) : '') + '>') : '') +
                    '<img toUpload="' + toUpload + '" align="' + align + '" width="' + width + '" height="' + height + '" _url="' + url + '" class="edui-faked-music"' +
                    ' src="' + me.options.langPath + me.options.lang + '/images/music.png" />' +
                    (addParagraph ? '</p>' : '')
          :
          '<object toUpload="' + toUpload + '" data="' + commonParams.dodoDevPath + '/statics/dewplayer/dewplayer-vol.swf" width="' + width + '" height="' + height + '" name="dewplayer" id="dewplayer" type="application/x-shockwave-flash">' + '<param name="movie" value="' + commonParams.dodoDevPath + '/statics/dewplayer/dewplayer-vol.swf" />' + '<param name="flashvars" value="mp3=' + url + '" />' + '<param name="wmode" value="transparent" />' + '</object>';
            }
            else {
                a = !toEmbed ?
                (addParagraph ? ('<p ' + (align != "none" ? (align == "center" ? ' style="text-align:center;" ' : ' style="float:"' + align) : '') + '>') : '') +
                    ...
                    (addParagraph ? '</p>' : '')
                :
                '<embed toUpload="' + toUpload + '" type="application/x-shockwave-flash" class="edui-faked-music" pluginspage="http://www.macromedia.com/go/getflashplayer"' + ' src="' + url + '" width="' + width + '" height="' + height + '" align="' + align + '"' +(align != "none" ? ' style= "' + (align == "center" ? "display:block;" : " float: " + align) + '"' : '') + ' wmode="transparent" play="true" loop="false" menu="false" allowscriptaccess="never" allowfullscreen="true" >';
            }
            return a;
        }
        ...
        me.commands["music"] = {
            execCommand: function (cmd, videoObjs, toUpload) {//toUpload判断是否是本地上传或是从网盘选取
                ...
                for (var i = 0, vi, len = videoObjs.length; i < len; i++) {
                    vi = videoObjs[i];
                    html.push(creatInsertStr(vi.url, vi.width || 400, vi.height || 95, vi.align || "none", false, true, toUpload));
                }
                me.execCommand("inserthtml", html.join(""));
            },
            queryCommandState: function () {
    ...
    (function () {
        ...
        Button.prototype = {
            ...
            getHtmlTpl: function () {
                var title = this.disabled ? "请使用IE9、chrome、firefox等以上版本使用涂鸦功能" : this.title;
                return '<div id="##" class="edui-box %%">' +
                    '<div id="##_state" stateful>' +
                     '<div class="%%-wrap"><div id="##_body" unselectable="on" ' + (this.title ? 'title="' + title + '"' : '') +
    ...
    (function () {
        ...
        var iframeUrlMap = {
            ...
            'thumbnail': '~/dialogs/thumbnail/thumbnail.html'
        };
        ...
        var dialogBtns = {
            noOk: ['searchreplace', 'help', 'webapp'],
            ok: ['attachment', 'anchor', 'link', 'insertimage', 'map', 'gmap', 'insertframe', 'wordimage',
                'insertvideo', 'highlightcode', 'insertframe', 'edittip', 'edittable', 'edittd', 'scrawl', 'template', 'music', 'background', 'spechars', 'thumbnail']
        ...
        //更多功能
        editorui.morefunction = function (editor, title) {
            title = editor.options.labelMap['morefunction'] || editor.getLang("labelMap.morefunction") || '';
            var ui = new editorui.Button({
                className: 'edui-for-morefunction-down',
                title: title,
                theme: editor.options.theme,
                onclick: function () {
                    if ($(".edui-toolbar").height() != 24) {
                        $(".edui-toolbar").css("height", "24px");
                        //$(".edui-for-morefunction-up")[0].className = "edui-for-morefunction-down";
                        $(".edui-for-morefunction-up").addClass("edui-for-morefunction-down").removeClass("edui-for-morefunction-up");
                    }
                    else {
                        $(".edui-toolbar").css("height", "auto");
                        //$(this)[0].className = "edui-for-morefunction-up"
                        $(".edui-for-morefunction-down").addClass("edui-for-morefunction-up").removeClass("edui-for-morefunction-down");
                    }
                    //if (editor.ui) {
                    //    editor.ui.setFullScreen(!editor.ui.isFullScreen());
                    //}
            ...
            editorui.buttons['morefunction'] = ui;
            editor.addListener('selectionchange', function () {
                var state = editor.queryCommandState('morefunction');
                ui.setDisabled(state == -1);
                ui.setChecked(editor.ui.isFullScreen());
            });
            return ui;
