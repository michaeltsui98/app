...
include_once COLA_DIR . '/Com/WebServices/Hessian/HessianFactory.php';
include_once COLA_DIR . '/Com/WebServices/Hessian/HessianTransport.php';

/**
 * Proxy to issue RPC calls to remote Hessian services
 */
class Cola_Com_WebServices_Hessian extends Cola_Com_WebServices_Abstract
{
    ...
    {
        $params += array('host' => '127.0.0.1', 'port' => 80, 'options' => null);
        parent::__construct($params);
        $this->url = 'http://' . $params['host'] . ':' . $params['port'] . $params['services'];
        $this->options = HessianOptions::resolveOptions($params['options']);
    ...
    {
        if (strpos($method, "__") === 0) throw new HessianException("Cannot call methods that start with __");
        ...
        if ($ctx->error instanceof Exception) throw $ctx->error;
        return $result;
    }
