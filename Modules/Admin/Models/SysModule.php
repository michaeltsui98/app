...
class Modules_Admin_Models_SysModule extends Cola_Model {
	...
	public function getMenu($group_id){
	    ...
	    if($group_id==0){
	        $sql = "SELECT * FROM sys_module where module_isok=1  ORDER BY module_order asc";
	    }else{
	 ...
	 * @param string $xk
	 */
	public function getModuleList($module_pid){
		 
		$where = " and module_pid = '$module_pid' ";	
		$sql = "SELECT * FROM `sys_module` where 1 $where  order by module_order asc";
		return $this->sql($sql);
	}
	/**
	 * 子模块的统计
	 */
	public function getSubModuleCount(){
		$sql = "SELECT b.module_pid,count(*) c from sys_module b 
			where   b.module_pid in(
			select a.module_id from sys_module a   
			 )
	 ...
	 * @return bool | int
	 */
	public function checkModuleTitleAction($title){
		$res = false;
		$res = $this->count("module_title = '$title' ");
		return $res;
	}
	
	public function getTreeList( $page,$rows,$where, $order, $type){
	    $sql = "select * from {$this->_table}  where  $where  order by  $order";
