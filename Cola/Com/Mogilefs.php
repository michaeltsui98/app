...
class Cola_Com_Mogilefs
{
                    ...
                    switch ($words[1]) {
                        case 'unknown_key':
                            if (isset($args['from_key'])) {
                                ...
                                throw (new MogileFsNotFoundException($args['key']));
                            }
                            break;
                        case 'empty_file':
                            throw (new MogileFsFileEmptyException($args['key']));
                            break;
                        default:
    ...
    public function exists($key)
    {
        ...
        } catch (MogileFsNotFoundException $e) {
            return false;
        }
    ...
    {
        if ($key === null) throw new Exception(get_class($this) . "::getPassthru key cannot be null");
        $paths = $this->getPaths($key);
        foreach ($paths as $path) {
            $fh = fopen($path, 'r');
            if ($fh) {
                if (fpassthru($fh) === false) throw new Exception(get_class($this) . "::getPassthru failed");
                fclose($fh);
            }
            return $success;
        }
