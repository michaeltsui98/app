...
class Modules_Admin_Models_SysUser extends Cola_Model {
	 ...
	 * 
	 */
	public  function getUserList($page,$rows,$group_id){
	    $where = "";
		if ( $group_id>0 ) {
	...
	public  function checkUserName($user_name){
	     $status = false;
	     $status = $this->count("user_name = '$user_name' ");
	     return $status;
	}
	/**
	 * 检查用户是否是当前学科的管理员
	 ...
	 */
	public function checkAdminByUid($user_id){
		$status = false;
		$status = $this->count("user_uid = '$user_id' ");
		return $status;
	}
