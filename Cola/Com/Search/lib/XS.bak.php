 ...
 *
 * 切勿手动修改本文件！生成时间：2012/07/02 13:21:40
 *
 * @author hightman
...
class XS extends XSComponent
{
    ...
    public function setScheme(XSFieldScheme $fs)
    {
        $fs->checkValid(true);
        $this->_scheme = $fs;
        if ($this->_search !== null) $this->_search->markResetScheme();
    }
    ...
    public function restoreScheme()
    {
        if ($this->_scheme !== $this->_bindScheme) {
            ...
            if ($this->_search !== null) $this->_search->markResetScheme(true);
        }
    }
    ...
    public static function convert($data, $to, $from)
    {
        ...
        if (is_string($data) && preg_match('/[\x81-\xfe]/', $data)) {
            if (function_exists('mb_convert_encoding')) return mb_convert_encoding($data, $to, $from);
            else if (function_exists('iconv')) return iconv($from, $to . '//TRANSLIT', $data);
            else throw new Cola_Exception('Cann\'t find the mbstring or iconv extension to convert encoding');
        }
        return $data;
    }
    ...
    private function loadIniFile($file)
    {
        ...
        if ($this->_config === false) throw new Cola_Exception('Failed to parse project config file/string: \'' . substr($file, 0, 10) . '...\'');
        $scheme = new XSFieldScheme;
        foreach ($this->_config as $key => $value) {
            if (is_array($value)) $scheme->addField($key, $value);
        }
        $scheme->checkValid(true);
        if (!isset($this->_config['project.name'])) $this->_config['project.name'] = basename($file, '.ini');
...
class XSDocument implements ArrayAccess, IteratorAggregate
{
    ...
    public function __construct($p = null, $d = null)
    {
        ...
        if ($d !== null && is_string($d)) $this->setCharset($d);
    }
    ...
    public function __set($name, $value)
    {
        if ($this->_meta !== null) throw new Cola_Exception('Magick property of result document is read-only');
    ...
    public function __call($name, $args)
    {
        if ($this->_meta !== null) {
            ...
            if (isset($this->_meta[$name])) return $this->_meta[$name];
        }
        throw new Cola_Exception('Call to undefined method `' . get_class($this) . '::' . $name . '()\'');
    }
    ...
    public function setCharset($charset)
    {
        ...
        if ($this->_charset == 'UTF8') $this->_charset = 'UTF-8';
    }

    public function setFields($data)
    {
        if ($data === null) $this->_data = array();
        else $this->_data = array_merge($this->_data, $data);
    }

    public function setField($name, $value)
    {
        if ($value === null) unset($this->_data[$name]);
    ...
    {
        $field = strval($field);
        if ($this->_terms === null || !isset($this->_terms[$field])) return null;
        $terms = array();
    ...
    {
        $field = strval($field);
        if ($this->_texts === null || !isset($this->_texts[$field])) return null;
    ...
    public function addTerm($field, $term, $weight = 1)
    {
        ...
        if (!isset($this->_terms[$field])) $this->_terms[$field] = array($term => $weight);
        else if (!isset($this->_terms[$field][$term])) $this->_terms[$field][$term] = $weight;
        else $this->_terms[$field][$term] += $weight;
    }

    public function addIndex($field, $text)
    {
        $field = strval($field);
        if (!is_array($this->_texts)) $this->_texts = array();
        if (!isset($this->_texts[$field])) $this->_texts[$field] = strval($text);
        else $this->_texts[$field] .= "\n" . strval($text);
    }
    ...
    {
        if (!is_null($name)) $this->__set(strval($name), $value);
    }
    ...
    {
        if ($this->_charset === null) $this->_charset = $index->xs->getDefaultCharset();
        return true;
    }
...
class XSFieldScheme implements IteratorAggregate
{
    ...
    public function getField($name, $throw = true)
    {
        if (is_int($name)) {
            if (!isset($this->_vnoMap[$name])) {
                if ($throw === true) throw new Cola_Exception('Not exists field with vno: `' . $name . '\'');
                return false;
            }
            $name = $this->_vnoMap[$name];
        }
        if (!isset($this->_fields[$name])) {
            if ($throw === true) throw new Cola_Exception('Not exists field with name: `' . $name . '\'');
            return false;
        }
        return $this->_fields[$name];
    }
    ...
    {
        if (!$field instanceof XSFieldMeta) $field = new XSFieldMeta($field, $config);
        if (isset($this->_fields[$field->name])) throw new Cola_Exception('Duplicated field name: `' . $field->name . '\'');
        if ($field->isSpeical()) {
            if (isset($this->_typeMap[$field->type])) {
                $prev = $this->_typeMap[$field->type];
                throw new Cola_Exception('Duplicated ' . strtoupper($config['type']) . ' field: `' . $field->name . '\' and `' . $prev . '\'');
            }
        ...
        if ($field->type == XSFieldMeta::TYPE_ID) $this->_fields = array_merge(array($field->name => $field), $this->_fields);
        else $this->_fields[$field->name] = $field;
    }

    public function checkValid($throw = false)
    {
        if (!isset($this->_typeMap[XSFieldMeta::TYPE_ID])) {
            if ($throw) throw new Cola_Exception('Missing field of type ID');
            return false;
        }
        return true;
    }
...
class XSFieldMeta
{
    ...
    public function __construct($name, $config = null)
    {
        ...
        if (is_array($config)) $this->fromConfig($config);
    }
    ...
    public function val($value)
    {
        if ($this->type == self::TYPE_DATE) {
            if (!is_numeric($value) || strlen($value) != 8) $value = date('Ymd', is_numeric($value) ? $value : strtotime($value));
        }
        return $value;
    }
    ...
    {
        if (isset(self::$_tokenizers[$this->tokenizer])) return self::$_tokenizers[$this->tokenizer];
        else {
            ...
            if (!class_exists($name)) throw new Cola_Exception('Undefined custom tokenizer `' . $this->tokenizer . '\' for field `' . $this->name . '\'');
            $obj = $arg === null ? new $name : new $name($arg);
            if (!$obj instanceof XSTokenizer) throw new Cola_Exception($name . ' for field `' . $this->name . '\' dose not implement the interface: XSTokenizer');
    ...
    public function toConfig()
    {
        ...
        else if ($this->type === self::TYPE_DATE) $str .= "type = date\n";
        else if ($this->type === self::TYPE_ID) $str .= "type = id\n";
        else if ($this->type === self::TYPE_TITLE) $str .= "type = title\n";
        else if ($this->type === self::TYPE_BODY) $str .= "type = body\n";
        if ($this->type !== self::TYPE_BODY && ($index = ($this->flag & self::FLAG_INDEX_BOTH))) {
            if ($index === self::FLAG_INDEX_BOTH) {
                if ($this->type !== self::TYPE_TITLE) $str .= "index = both\n";
            }
            else if ($index === self::FLAG_INDEX_MIXED) {
                $str .= "index = mixed\n";
            } else {
                if ($this->type != self::TYPE_ID) $str .= "index = self\n";
            }
        }
        if ($this->type !== self::TYPE_ID && $this->tokenizer !== XSTokenizer::DFL) $str .= "tokenizer = " . $this->tokenizer . "\n";
        if ($this->cutlen > 0 && !($this->cutlen === 300 && $this->type === self::TYPE_BODY)) $str .= "cutlen = " . $this->cutlen . "\n";
        if ($this->weight !== 1 && !($this->weight === 5 && $this->type === self::TYPE_TITLE)) $str .= "weight = " . $this->weight . "\n";
        if ($this->flag & self::FLAG_WITH_POSITION) {
            if ($this->type !== self::TYPE_BODY && $this->type !== self::TYPE_TITLE) $str .= "phrase = yes\n";
        }
        else {
            if ($this->type === self::TYPE_BODY || $this->type === self::TYPE_TITLE) $str .= "phrase = no\n";
        }
        return $str;
    }
    ...
    public function fromConfig($config)
    {
        ...
        if (isset($config['index']) && $this->type != self::TYPE_BODY) {
            ...
            if ($this->type == self::TYPE_ID) $this->flag |= self::FLAG_INDEX_SELF;
        }
        if (isset($config['cutlen'])) $this->cutlen = intval($config['cutlen']);
        if (isset($config['weight']) && $this->type != self::TYPE_BODY) $this->weight = intval($config['weight']) & self::MAX_WDF;
        if (isset($config['phrase'])) {
            if (!strcasecmp($config['phrase'], 'yes')) $this->flag |= self::FLAG_WITH_POSITION;
            else if (!strcasecmp($config['phrase'], 'no')) $this->flag &= ~ self::FLAG_WITH_POSITION;
        }
...
class XSIndex extends XSServer
{
    ...
    {
        if ($doc->beforeSubmit($this) === false) return $this;
        $fid = $this->xs->getFieldId();
        $key = $doc->f($fid);
        if ($key === null || $key === '') throw new Cola_Exception('Missing value of primarky key (FIELD:' . $fid . ')');
        ...
        foreach ($this->xs->getAllFields() as $field) /* @var $field XSFieldMeta */ {
            if (($value = $doc->f($field)) !== null) {
                ...
                if (!$field->hasCustomTokenizer()) {
                    ...
                    if ($field->hasIndexMixed()) $cmds[] = new XSCommand(CMD_DOC_INDEX, $wdf, XSFieldScheme::MIXED_VNO, $value);
                    if ($field->hasIndexSelf()) {
                        $wdf |= $field->isNumeric() ? 0 : CMD_INDEX_FLAG_SAVEVALUE;
                        $cmds[] = new XSCommand(CMD_DOC_INDEX, $wdf, $field->vno, $value);
                    }
                    if (!$field->hasIndexSelf() || $field->isNumeric()) $cmds[] = new XSCommand(CMD_DOC_VALUE, $varg, $field->vno, $value);
                }
                else {
                    if ($field->hasIndex()) {
    ...
    public function del($term, $field = null)
    {
        ...
        if ($this->_bufSize > 0) $this->appendBuffer(implode('', $cmds));
        else if (count($cmds) == 1) $this->execCommand($cmds[0], CMD_OK_RQST_FINISHED);
        else {
    ...
    {
        if (strlen($data) < 255 && $check_file && file_exists($data) && ($data = file_get_contents($data)) === false) throw new Cola_Exception('Failed to read exdata from file');
        ...
        if ($first != CMD_IMPORT_HEADER && $first != CMD_INDEX_REQUEST && $first != CMD_INDEX_SYNONYMS && $first != CMD_INDEX_REMOVE && $first != CMD_INDEX_EXDATA) {
            throw new Cola_Exception('Invalid start command of exdata (CMD:' . $first . ')');
        }
    ...
    public function addSynonym($raw, $synonym)
    {
        ...
        if ($raw !== '' && $synonym !== '') {
            ...
            if ($this->_bufSize > 0) $this->appendBuffer(strval($cmd));
            else $this->execCommand($cmd, CMD_OK_RQST_FINISHED);
        }
        return $this;
    }
    ...
    {
        if ($this->_buf !== '') $this->addExdata($this->_buf, false);
    ...
    public function flushLogging()
    {
        try {
            ...
            if ($e->getCode() === CMD_ERR_BUSY) return false;
            throw $e;
        }
        return true;
    }
    ...
    public function flushIndex()
    {
        try {
            ...
            if ($e->getCode() === CMD_ERR_BUSY || $e->getCode() === CMD_ERR_RUNNING) return false;
            throw $e;
        }
        return true;
    }
    ...
    public function __destruct()
    {
        if ($this->_rebuild === true) {
            try {
                ...
                throw new Cola_Exception('XS Index destruct error:' . $e->getMessage());
            }
        }
        parent::__destruct();
    }
...
class XSSearch extends XSServer
{
    ...
    public function setAutoSynonyms($value = true)
    {
        ...
        if ($value === true) $flag |= CMD_PARSE_FLAG_AUTO_MULTIWORD_SYNONYMS;
    ...
    {
        if (!is_array($fields)) return $this->setSort($fields);
        $buf = '';
        foreach ($fields as $key => $value) {
            ...
            if ($vno != XSFieldScheme::MIXED_VNO) $buf .= chr($vno) . chr($asc ? 1 : 0);
        }
        if ($buf !== '') {
    ...
    {
        if (is_array($field)) return $this->setMultiSort($field);
        if ($field === null) $cmd = new XSCommand(CMD_SEARCH_SET_SORT, CMD_SORT_TYPE_RELEVANCE);
        else {
    ...
    public function addRange($field, $from, $to)
    {
        if ($from !== null || $to !== null) {
            if (strlen($from) > 255 || strlen($to) > 255) throw new Cola_Exception('Value of range is too long');
            $vno = $this->xs->getField($field)->vno;
            $from = XS::convert($from, 'UTF-8', $this->_charset);
            $to = XS::convert($to, 'UTF-8', $this->_charset);
            if ($from === null) $cmd = new XSCommand(CMD_QUERY_VALCMP, CMD_QUERY_OP_FILTER, $vno, $to, chr(CMD_VALCMP_LE));
            else if ($to === null) $cmd = new XSCommand(CMD_QUERY_VALCMP, CMD_QUERY_OP_FILTER, $vno, $from, chr(CMD_VALCMP_GE));
            else $cmd = new XSCommand(CMD_QUERY_RANGE, CMD_QUERY_OP_FILTER, $vno, $from, $to);
            $this->execCommand($cmd);
        }
        return $this;
    }
    ...
    public function setFacets($field, $exact = false)
    {
        ...
        foreach ($field as $name) {
            ...
            if ($ff->type !== XSFieldMeta::TYPE_STRING) throw new Cola_Exception("Field `$name' cann't be used for facets search, can only be string type");
            $buf .= chr($ff->vno);
        }
    ...
    {
        if ($field === null) return $this->_facets;
    ...
    {
        $query = $query === null ? '' : $this->preQueryString($query);
        if ($query === '' && $this->_terms !== null) $ret = $this->_terms;
        else {
            ...
            for ($i = 0; $i < count($tmps); $i++) {
                if ($tmps[$i] === '' || strpos($tmps[$i], ':') !== false) continue;
                $ret[] = $tmps[$i];
            }
            if ($query === '') $this->_terms = $ret;
        }
    ...
    {
        $query = $query === null ? '' : $this->preQueryString($query);
        if ($query === '' && $this->_count !== null) return $this->_count;
        ...
        if ($query === '') $this->_count = $ret['count'];
        return $ret['count'];
    }

    public function search($query = null)
    {
        if ($this->_curDb !== self::LOG_DB) $this->_highlight = $query;
        $query = $query === null ? '' : $this->preQueryString($query);
            ...
            if ($res->cmd == CMD_SEARCH_RESULT_FACETS) {
                ...
                while (($off + 6) < strlen($res->buf)) {
                    ...
                    if (isset($vnoes[$tmp['vno']])) {
                        ...
                        if (!isset($this->_facets[$name])) $this->_facets[$name] = array();
    ...
    public function getHotQuery($limit = 6, $type = 'total')
    {
        ...
        try {
            ...
            if ($type !== 'lastnum' && $type !== 'currnum') $type = 'total';
            ...
            if ($e->getCode() != CMD_ERR_XAPIAN) throw $e;
        }
    ...
    public function getRelatedQuery($query = null, $limit = 6)
    {
        ...
        if ($query === null) $query = $this->cleanFieldQuery($this->_query);
        if (empty($query) || strpos($query, ':') !== false) return $ret;
        $op = $this->_defaultOp;
        $this->xs->setScheme(XSFieldScheme::logger());
        try {
            ...
            foreach ($result as $doc) /* @var $doc XSDocument */ {
                ...
                $body = $doc->body;
                if (!strcasecmp($body, $query)) continue;
                $ret[] = $body;
    ...
    public function getCorrectedQuery($query = null)
    {
        ...
        try {
            if ($query === null) {
                if ($this->_count > 0 && $this->_count > ceil($this->getDbTotal() * 0.001)) return $ret;
    ...
    public function highlight($value)
    {
        ...
        if (!is_array($this->_highlight)) $this->initHighlight();
        if (isset($this->_highlight['pattern'])) $value = preg_replace($this->_highlight['pattern'], $this->_highlight['replace'], $value);
        if (isset($this->_highlight['pairs'])) $value = str_replace(array_keys($this->_highlight['pairs']), array_values($this->_highlight['pairs']), $value);
        return $value;
    }

    private function logQuery($query = null)
    {
        if ($query !== '' && $query !== null) $terms = $this->terms($query, false);
        else {
            $query = $this->_query;
        ...
        foreach ($terms as $term) {
            ...
            if (($pos2 = strpos($query, $term, $pos1)) === false) continue;
            if ($pos2 === $pos) $log .= $term;
            else if ($pos2 < $pos) $log .= substr($term, 3);
            else {
                if (++$max > 3 || strlen($log) > 42) break;
                $log .= ' ' . $term;
            }
            $pos = $pos2 + strlen($term);
        }
        $log = trim($log);
        if (strlen($log) < 2 || (strlen($log) == 3 && ord($log[0]) > 0x80)) return;
    ...
    {
        $query = trim($query);
        if ($this->_resetScheme === true) $this->clearQuery();
        $newQuery = '';
        $parts = preg_split('/[ \t\r\n]+/', $query);
        foreach ($parts as $part) {
            ...
            if (($pos = strpos($part, ':', 1)) !== false) {
                ...
                if (($field = $this->xs->getField($name, false)) !== false && $field->vno != XSFieldScheme::MIXED_VNO) {
                    ...
                    if (!$field->isBoolIndex() && substr($part, $pos + 1, 1) != '(' && preg_match('/[\x81-\xfe]/', $part)) {
                        $newQuery .= substr($part, 0, $pos + 1) . '(' . substr($part, $pos + 1) . ')';
                    } else if ($field->isBoolIndex()) {
                        $value = substr($part, $pos + 1);
                        if (!$field->hasCustomTokenizer()) $newQuery .= substr($part, 0, $pos + 1) . strtolower($value);
                        else {
                            $terms = array();
            ...
            if (($part[0] == '+' || $part[0] == '-') && $part[1] != '(' && preg_match('/[\x81-\xfe]/', $part)) {
    ...
    {
        if (($field = $this->xs->getField($match[2], false)) === false) return $match[0];
        if ($field->isBoolIndex()) return '';
        if (substr($match[3], 0, 1) == '(' && substr($match[3], -1, 1) == ')') $match[3] = substr($match[3], 1, -1);
    ...
    private function initHighlight()
    {
        ...
        for ($i = 0; $i < count($tmps); $i++) {
            ...
            for ($j = $i + 1; $j < count($tmps); $j++) {
                if (strlen($tmps[$j]) !== 6 || substr($tmps[$j], 0, 3) !== substr($tmps[$j - 1], 3, 3)) break;
            }
            if (($k = ($j - $i)) === 1) $terms[] = XS::convert($tmps[$i], $this->_charset, 'UTF-8');
            else {
                $i = $j - 1;
                while ($k--) {
                    $j--;
                    if ($k & 1) $terms[] = XS::convert(substr($tmps[$j - 1], 0, 3) . $tmps[$j], $this->_charset, 'UTF-8');
        ...
        foreach ($terms as $term) {
            if (!preg_match('/[a-zA-Z]/', $term)) $pairs[$term] = '<em>' . $term . '</em>';
            else {
        ...
        if (count($pairs) > 0) $this->_highlight['pairs'] = $pairs;
        if (count($pattern) > 0) {
...
class XSCommand extends XSComponent
{
    ...
    public function __construct($cmd, $arg1 = 0, $arg2 = 0, $buf = '', $buf1 = '')
    {
        if (is_array($cmd)) {
            foreach ($cmd as $key => $value) {
                if ($key === 'arg' || property_exists($this, $key)) $this->$key = $value;
            }
        }
        else {
            $this->cmd = $cmd;
    ...
    public function __toString()
    {
        if (strlen($this->buf1) > 0xff) $this->buf1 = substr($this->buf1, 0, 0xff);
...
class XSServer extends XSComponent
{
    ...
    public function open($conn)
    {
        ...
        $this->connect();
        $this->_flag ^= self::BROKEN;
        if ($this->xs instanceof XS) $this->setProject($this->xs->getName());
    }

    public function reopen($force = false)
    {
        if ($this->_flag & self::BROKEN || $force === true) $this->open($this->_conn);
        return $this;
    }
    ...
    {
        if (!$cmd instanceof XSCommand) $cmd = new XSCommand($cmd);
        if ($cmd->cmd & 0x80) {
        ...
        $this->_sendBuffer = '';
        $this->write($buf);
        if ($this->_flag & self::FILE) return true;
        $res = $this->getRespond();
        if ($res->cmd === CMD_ERR && $res_cmd != CMD_ERR) throw new Cola_Exception($res->buf, $res->arg);
        if ($res->cmd != $res_cmd || ($res_arg != CMD_NONE && $res->arg != $res_arg)) throw new Cola_Exception('Unexpected respond {CMD:' . $res->cmd . ', ARG:' . $res->arg . '}');
        return $res;
    }
    ...
    {
        if ($this->_sock === null || $this->_flag & (self::BROKEN | self::FILE)) return false;
        $wfds = $xfds = array();
    ...
    {
        $buf = strval($buf);
        if ($len == 0 && ($len = $size = strlen($buf)) == 0) return true;
        $this->check();
        while (true) {
            $bytes = fwrite($this->_sock, $buf, $len);
            if ($bytes === false || $bytes === 0 || $bytes === $len) break;
            $len -= $bytes;
    ...
    private function read($len)
    {
        ...
        for ($buf = '', $size = $len;;) {
            ...
            if ($bytes === false || strlen($bytes) == 0) break;
            $len -= strlen($bytes);
            $buf .= $bytes;
            if ($len === 0) return $buf;
        }
    ...
    {
        if ($this->_sock === null) throw new Cola_Exception('No server connection');
        if ($this->_flag & self::BROKEN) throw new Cola_Exception('Broken server connection');
    }
    ...
    private function connect()
    {
        ...
        if (is_int($conn) || is_numeric($conn)) {
            ...
            $conn = substr($conn, 7);
            if (($sock = @fopen($conn, 'wb')) === false) throw new Cola_Exception('Failed to open local file for writing: `' . $conn . '\'');
        ...
        if (($sock = @fsockopen($host, $port, $errno, $error, 5)) === false) throw new Cola_Exception($error . '(C#' . $errno . ')');
...
class XSTokenizerSplit implements XSTokenizer
{
    ...
    {
        if ($arg !== null && $arg !== '') $this->arg = $arg;
    }

    public function getTokens($value, XSDocument $doc = null)
    {
        if (strlen($this->arg) > 2 && substr($this->arg, 0, 1) == '/' && substr($this->arg, -1, 1) == '/') return preg_split($this->arg, $value);
...
class XSTokenizerXlen implements XSTokenizer
{
    ...
    public function __construct($arg = null)
    {
        if ($arg !== null && $arg !== '') {
            $this->arg = intval($arg);
            if ($this->arg < 1 || $this->arg > 255) throw new Cola_Exception('Invalid argument for ' . __CLASS__ . ': ' . $arg);
        }
    }
...
class XSTokenizerScws implements XSTokenizer
{
    ...
    public function __construct()
    {
        if (self::$_server === null) {
            $xs = XS::getLastXS();
            if ($xs === null) throw new Cola_Exception('An XS instance should be created before using ' . __CLASS__);
