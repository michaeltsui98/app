...
class Cola_Model
{
    ...
    public static function init(){
        ...
        if(!isset(self::$_instance[$cls]) or self::$_instance[$cls] === NULL){
    ...
    function  getWhere($where=array()){
        if($where != null ) {
           ...
           // $where  = array_filter($where);
            ...
            {
                    if($value == null){
                        $wsql .=($wsql == '')?"  (" . $key ." is null or $key = 0)": " and (" . $key . " is null or $key = 0)";
                    }elseif($value == 0){
                        $wsql .=($wsql == '')?"  " . $key ."= 0": " and " . $key . " = 0";
                    }else{
                        $wsql .=($wsql == '')?" " . $key . "='" . $value . "'" : " and " . $key . "='" . $value . "'";
                    }
