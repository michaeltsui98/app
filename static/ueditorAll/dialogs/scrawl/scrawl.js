...
(function () {
    ...
    scrawl.prototype = {
        ...
        originalState:function (options) {
            ...
            $G("fileForm").action = url;//初始form提交地址
        ...
        getCanvasData: function () {
            ...
            if (img) {
                ...
                if (img.style.position == "absolute") {
                    x = parseInt(img.offsetLeft);
                    y = parseInt(img.offsetTop);
                } else {
...
function exec(scrawlObj) {
    if (scrawlObj.isScrawl) {
        ...
        if (!!base64) {
            ajax.request(editor.options.scrawlSaveUrl + "?app=" + editor.options.forModule, {
                timeout:100000,
                content:base64,
                onsuccess:function (xhr) {
                    if (!scrawlObj.isCancelScrawl) {
                        ...
                        if (responseObj.state == "SUCCESS") {
                            ...
                            var richtextboxid = "";
                            if ($("#richtextbox", parent.document).attr("richtextboxid") == "") {
                                richtextboxid = responseObj.id;
                            }
                            else {
                                richtextboxid = $("#richtextbox", parent.document).attr("richtextboxid") + "," + responseObj.id;
                            }
                            $("#richtextbox", parent.document).attr("richtextboxid", richtextboxid);
                            dialog.close();
                        } else {
